...
function move(enemy, type, speed, args){
	...
	if (type == 1){
		...
		if (enemy.param == undefined){
			enemy.radius = Math.sqrt((enemy.x - args[0]) * (enemy.x - args[0]) +
				(enemy.y - args[1]) * (enemy.y - args[1]));
			enemy.param = Math.acos((enemy.x - args[1])/enemy.radius);
		}
		else{
			enemy.x = enemy.radius * 0.66 * Math.cos(enemy.param) + args[0];
			enemy.y = enemy.radius * 0.66 * Math.sin(enemy.param) + args[1];
			enemy.angle = enemy.param/Math.PI * 180;
		}
	}
	else if (type == 2){
		// move up down
